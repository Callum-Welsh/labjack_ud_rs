...
static const long LJ_chTDAC_READ_CAL_CONSTANTS = 5123; // UE9 + U3 + U6
...
static const LJ_ERROR LJE_INVALID_CHANNEL_NUMBER = 2; // Occurs when a channel that doesn't exist is specified (i.e. DAC #2 on a UE9), or data from streaming is requested on a channel that isn't streaming.
...
static const LJ_ERROR LJE_BUFFER_OVERRUN = 8; // Occurs when stream buffer overruns (this is the driver buffer not the hardware buffer).  Stream is stopped.
